...
export default function ShipmentsIncOut() {

  const name = useLanding((state) => state.name);
  const text = useShipIncOut((state) => state.text1);
  const setText = useShipIncOut((state) => state.setName1);
  const setBool = useShipIncOut((state) => state.setBool1);
    ...
    <section className="w-full h-screen my-auto flex flex-row">
      ...
      <aside className="w-1/3 h-5/6 my-auto flex flex-col justify-evenly content-between px-5">
        ...
        <footer className="h-fit w-full mb-10">
          <p className="text-[#5e5e5e] text-sm mb-1">Step 2 of 7</p>
          <div className="w-11/12 h-2 rounded-full bg-white">
            <div className="w-3/12 h-full rounded-full bg-[#7B5DE8]"></div>
          </div>
        </footer>
      </aside>
